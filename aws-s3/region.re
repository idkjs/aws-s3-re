...
type t =
  ...
  | Other(string) /* Other unknown */
  | Vendor(vendor); /* S3-compatible vendor/service */
...
let of_host = host =>
  switch (String.split_on_char(~sep='.', host) |> List.rev) {
  | ["com", "amazonaws", "s3", ..._] => Us_east_1
  | ["com", "amazonaws", host, ..._]
      when
        ...
        && host.[1] == '3'
        && host.[2] == '-' =>
    String.sub(~pos=3, ~len=String.length(host) - 3, host) |> of_string
  | ["com", "amazonaws", host, ..._] => host |> of_string
...
let to_host = (~dualstack, region) =>
  ...
  | Vendor(v) => v.host
  | _ =>
    let dualstack = dualstack ? ".dualstack" : "";
