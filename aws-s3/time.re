...
let parse_iso8601_string = str =>
  ...
  |> (
    fun
    | [@implicit_arity] Ok(t, _, _) => Ptime.to_float_s(t)
    | Error(_) => failwith("Could not parse date string")
  );

let%test _ = parse_iso8601_string("2018-02-27T13:39:35.000Z") == 1519738775.0;
...
let parse_rcf1123_string = date_str => {
  ...
  Scanf.sscanf(
    date_str,
    "%s %d %s %d %d:%d:%d %s",
    (_dayname, day, month_str, year, hour, min, sec, _zone) => {
      let month = int_of_month(month_str);
      switch (
...
let iso8601_of_time = time => {
  let t =
    ...
    |> (
      fun
      | Some(t) => t
      | None => failwith("Time out of range")
    );

  let ((year, month, day), ((hour, min, sec), _)) = Ptime.to_date_time(t);
  let date_str = sprintf("%.4d%.2d%.2d", year, month, day);
  let time_str = sprintf("%.2d%.2d%.2d", hour, min, sec);
  (date_str, time_str);
};
